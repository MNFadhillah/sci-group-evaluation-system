        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                                    ...
                                    <tbody id="modalQuestionList">
                                        @foreach ($questions as $q)
                                            ...
                                            <tr data-qid="{{ $q->id }}" id="modalRow-{{ $q->id }}">
                                                ...
                                                <td>{{ $q->type }}</td>
                                                <td>{{ $q->difficulty }}</td>
                                                <td style="white-space:normal;">{{ Str::limit($qData->text ?? '-', 300) }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                ...
                <div class="modal-footer">
                    <div class="me-auto text-muted small">Pilih soal lalu tekan <strong>Terapkan ke Aktivitas</strong>
                    </div>
    ...
    <script>
        ...
        async function renderSelectedArea(ids, questionsMap = null) {
            ...
            await Promise.all(toFetch.map(async id => {
                ...
                if (q && q.text) {
                    ...
                    if (smallEl && q.difficulty && q.type) smallEl.innerText =
                        `${q.difficulty} — ${q.type}`;
                } else {
                    const el = document.getElementById(`selectedText-${id}`);
        ...
        async function renderModalSelected(questionsMap = null) {
            ...
            if (!modalSelected || modalSelected.length === 0) {
                wrap.innerHTML =
                    `<div class="text-center text-muted py-4" id="modalNoSelected">Belum ada pilihan di modal.</div>`;
                return;
            }
        ...
        function modalToggleSelect(id) {
            ...
            if (modalSelected.includes(id)) {
                ...
                if (IS_MODE1 && modalSelected.length >= 1) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Tidak diizinkan',
                        text: 'Mode 1 hanya boleh memiliki 1 soal. Hapus pilihan sebelumnya dulu jika ingin mengganti.'
                    });
                    return;
                }
            ...
            btnApply.addEventListener('click', function() {
                ...
                if (IS_MODE1 && modalSelected.length !== 1) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Tidak diizinkan',
                        text: 'Mode 1 hanya boleh memiliki tepat 1 soal.'
                    });
                    return;
                }
                ...
                if (IS_MODE1) {
                    ...
                    jumlahSoal = parseInt(jumlahSoalEl.value);
                }
            ...
            fetch("{{ url('/guru/hapus-soal-manual/' . $aktivitas->id) }}", {
                ...
                .then(res => {
                    if (res.success) {
                        ...
                        document.getElementById('currentTotal') && (document.getElementById('currentTotal').innerText =
                            (window.lastPicked || []).length);
                        updateCounter();
                    } else {
                ...
                fetch("{{ url('/guru/clear-all/' . $aktivitas->id) }}", {
                    ...
                    .then(res => {
                        if (res.success) {
                            ...
                            document.getElementById('currentTotal') && (document.getElementById('currentTotal')
                                .innerText = 0);
                            updateCounter();
                ...
                if (e.key === ' ' || e.key === 'Enter') {
                    setTimeout(() => {
                        const checked = document.querySelector(
                            'input[name="modalJumlahRadio"]:checked');
                        if (checked) {
                            clearRadioActiveVisuals();
        ...
        function simpanPilihan() {
            ...
            if (isAdaptive) {
                if (!n || n <= 0) {
                    Swal.fire('Perlu memilih jumlah soal',
                        'Silakan pilih jumlah soal (radio) atau gunakan Ambil Soal Otomatis dulu.', 'warning');
                    return;
                }
                ...
                if (lackEasy > 0 || lackMedium > 0 || lackHard > 0 || lackTotal > 0) {
                    let parts = [];
                    if (lackTotal > 0) parts.push(
                        `Total soal kurang: <b>${lackTotal}</b> (dibutuhkan minimal ${totalRequired})`);
